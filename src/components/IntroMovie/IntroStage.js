import DynamicLine from './DynamicLine/index';
...
class IntroStage {
  constructor(canvas) {
    ...
    this.createjs.Ticker.setFPS(60);
  ...
  createImageLetter(img) {
    let letter = new this.createjs.Bitmap(img);
    ...
    let container = new this.createjs.Container();
    ...
    // let rect = new this.createjs.Shape();
  ...
  createPoint(x, y, color) {
    let graphics = new this.createjs.Graphics();
    graphics.setStrokeStyle(1);
    graphics.beginFill(color || 'red');
    graphics.drawCircle(x, y, 3);
    return new this.createjs.Shape(graphics);
  }
}
