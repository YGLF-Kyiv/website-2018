class LettersContainer {
  ...
  initAnimations() {
    ...
    this.letterTexts.forEach((letterText, index) => {
      ...
      const letterTween = this.createjs.Tween.get(letterText);
      letterTween
  ...
  createLettersContainer() {
    const lettersContainer = new this.createjs.Container();
  ...
  createLetterText(text, font) {
    const letterText = new this.createjs.Text(text, font, '#FFF');
